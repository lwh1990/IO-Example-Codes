...
/* We can get all these dimensions from the dataset in the file, but for now I'm setting them here. */
    ...
    status = H5Pset_cache(fapl, 0, 521, 3 * 1024 * 1024, 1);
    ...
    for (i=0; i<(dims[0]/count[0]); i++) {
        offset[0] = i * count[0];
        for (j=0; j<(dims[1]/count[1]); j++) {
            offset[1] = j * count[1];
            for (k=0; k<(dims[2]/count[2]); k++) {
                offset[2] = k * count[2];

                status = H5Sselect_hyperslab (space, H5S_SELECT_SET,
                                      offset, NULL, count, NULL);

                status = H5Dread (dset, H5T_NATIVE_FLOAT, dataspace, space, H5P_DEFAULT,
                          (void *) rdata);
            }
        }
        if (i%10 == 1) {
            printf("Hyperslab %d value (0,0):  %f\n", 270 * (i+1), rdata[0]);
        }
    } // i-loop
